...
def estimationTable(df, show = 'pval', stars = False, col_label = 'Est. Results', step = 0, check = 0):
    ...
    if check == 0:
        ...
        dictionary['G_hat_fd'] = 'Securitized Loan Sales'
    elif check == 2:
        dictionary['G_hat_fd'] = 'Non-securitized Loan Sales'
    
    # Get parameter column and secondary columns (std, tval, pval)
    params = df.Parameter.iloc[[i for i in range(1,12)] + [0]].round(4)
...
# Set path list
path_list_step1 = ['Robustness_checks/Step_1/Step1_robust_{}.csv'.format(i) for i in range(0,8,2)]
path_list_coff_step2 = ['Robustness_checks/Step_2/Step2_robust_{}.csv'.format(i) for i in range(0,8,2)]
path_list_npl_step2 = ['Robustness_checks/Step_2/Step2_robust_{}.csv'.format(i) for i in range(1,8,2)]  
