...
vars_needed = dep_vars + righthand_x.split(' + ') + vars_endo + list(np.unique(vars_z[0].split(' + '))) + ['dum_ls']
...
ids_atls = df_full[df_full.dum_ls == 1].index.get_level_values(0).value_counts()\
    [df_full[df_full.dum_ls == 1].index.get_level_values(0).value_counts() == 18].index.tolist()
df_full = df_full[df_full.index.get_level_values(0).isin(ids_atls)]
...
for i in range(len(res_step2_dumls[0])):
    for d in range(len(list_dfs)):
        res_s2_shuffled.append(res_step2[d][i])

sum_s2 = summary_col(res_s2_shuffled, show = 'se', regressor_order = reg_order_step2)
...
endo_sort = [vec[i] for i in range(len(pvals_ls_endo[0])) for vec in endo_dumls]

## Sargan test - step 2
### Make lists to append to table 2
'''NOTE: All uneven items are with hetero correction'''
sargan_sort = [vec[i] for i in range(len(sargan_res[0])) for vec in sargan_dumls]
...
adjrs = vecAdjR2(list(sum_step1.tables[2].iloc[2,:].astype(float)),\
                       list(sum_step1.tables[2].iloc[1,:].astype(float)), k_step1)  
    
## Change the lower part of the table
step1_lower_table = sum_step1.tables[2].iloc[[1],:]
step1_lower_table.loc[-1] = adjrs
step1_lower_table.rename({-1:'$Adj. R^2$'}, axis = 'index', inplace = True)
step1_lower_table.loc[-1] = f_test
step1_lower_table.rename({-1:'F-test Weak Instruments'}, axis = 'index', inplace = True)

### Add to the table
sum_step1.tables[2] = step1_lower_table

## Remove the time dummies
step1_param_table = sum_step1.tables[1].iloc[:-np.sum((sum_step1.tables[1].index.str.contains('dum') * 1))*2,:]
step1_param_table.loc[-1] = ['Yes'] * len(columns)
step1_param_table.rename({-1:'Time Dummies'}, axis = 'index', inplace = True)

### add to the table
sum_step1.tables[1] = step1_param_table

## Make new table
table_step1 = pd.concat(sum_step1.tables[1:3])

### Change the columns of the table
table_step1.columns = columns

### Change the index
table_step1.index = [dict_var_names[key] for key in table_step1.index]
...
## Calculate the adj. R2
k_step2 = [mod.model.exog.shape[0] for mod in res_step1] * len(dep_vars)

adjrs_step2 = vecAdjR2(list(sum_s2.tables[2].iloc[2,:].astype(float)),\
                       list(sum_s2.tables[2].iloc[1,:].astype(float)), k_step2)

## Change the lower part of the table
step2_lower_table = sum_s2.tables[2].iloc[[1],:]
step2_lower_table.loc[-1] = adjrs_step2
step2_lower_table.rename({-1:'$Adj. R^2$'}, axis = 'index', inplace = True)
step2_lower_table.loc[-1] = endo_sort
step2_lower_table.rename({-1:'DWH-test'}, axis = 'index', inplace = True)
step2_lower_table.loc[-1] =  sargan_sort
step2_lower_table.rename({-1:'P-val Sargan-test'}, axis = 'index', inplace = True)

### Add to the table
sum_s2.tables[2] = step2_lower_table

## Remove the time dummies
step2_param_table = sum_s2_.tables[1].iloc[:-np.sum((sum_s2.tables[1].index.str.contains('dum') * 1))*2,:]
step2_param_table.loc[-1] = ['Yes'] * len(columns) * len(dep_vars)
step2_param_table.rename({-1:'Time Dummies'}, axis = 'index', inplace = True)

### add to the table
sum_s2.tables[1] = step2_param_table

## Make new table
table_step2 = pd.concat(sum_s2.tables[1:3])
...
table_step2.columns = pd.MultiIndex.from_tuples(columns_step_2)

### Change the index
dict_var_names.update({'G_hat_fd':'Loan Sales/TA'})
table_step2.index = [dict_var_names[key] for key in table_step2.index]
...
# Save the tables
path = r'Results\FD_IV_v5_atls.xlsx'


with pd.ExcelWriter(path) as writer:
    table_step1.to_excel(writer, sheet_name = 'Step 1 LSTA') 
    
    table_step2.to_excel(writer, sheet_name = 'Step 2 LSTA')