...
''' Crisis dates are (based on the NBER recession dates):
    ...
    We split the sample on before and after crisis. We retain the full sample
    '''
    
df_pre = df_fd[df_fd.index.get_level_values(1) < pd.Timestamp(2010,12,31)]
...
def analysesFDIV(dep_var, var_ls, df, righthand_z, righthand_x):
    ...
    res_step2 = PanelOLS.from_formula(dep_var + '_lead' + ' ~ '  + 'G_hat_fd' + ' + ' +  righthand_x +\
                     ' + ' + time_dummies_step2  + ' + 1', data = df_lead).\
...
#----------------------------------------------
if __name__ == '__main__': 
    step1, step2, f, endo, sargan_res = zip(*Parallel(n_jobs = num_cores)(delayed(analysesFDIV)(dep_var, vars_endo, data, vars_z, righthand_x) for dep_var in dep_vars for data in list_dfs))
...
def step1ToCSV(table1, fval, i):    
    ...
    ## Save to csv
    main_table1.to_csv('Results\Lag\Step_1\Step1_lag_{}.csv'.format(i))
...
def step2ToCSV(table2, endo_val, sargan_val, i):    
    ...
    ## Save to csv
    main_table2.to_csv('Results\Lag\Step_2\Step2_lag_{}.csv'.format(i))
