...
def analysesFDIV(dep_var, var_ls, df, righthand_z, righthand_x):
    ...
    res_step2 = PanelOLS.from_formula(dep_var + '_lead' + ' ~ ' + 'G_hat_fd' + ' + ' + righthand_x +\
                     ' + ' + time_dummies_step2, data = df_lead).\
...
#----------------------------------------------
if __name__ == '__main__': 
    step1, step2, f, endo, sargan_res = zip(*Parallel(n_jobs = num_cores)(delayed(analysesFDIV)(dep_var, vars_endo, df_fd[df_fd.index.get_level_values(1).isin(list_yearindex[year_vec:year_vec+window])], vars_z, righthand_x) for dep_var in dep_vars for year_vec in range(1,t-window + 1)))
...
def step1ToCSV(table1, fval, i):    
    ...
    ## Save to csv
    main_table1.to_csv('Results\Rolling_average\Step_1\Step1_ra_lag_{}.csv'.format(i))
...
def step2ToCSV(table2, endo_val, sargan_val, i):    
    ...
    ## Save to csv
    main_table2.to_csv('Results\Rolling_average\Step_2\Step2_ra_lag_{}.csv'.format(i))
...
# Prelims
year_labels = [list_yearindex[i+int(np.ceil(window/2)) - 1] for i in range(t-window)] #subtract one year for the plots
...
def plotRollingAverage(params, std_errors, depvar, var_name, bar_year):
    ...
    ## Save the figure
    fig.savefig('Figures\Moving_averages\{}\MA_lag_{}_{}.png'.format(depvar,depvar,var_name))  
