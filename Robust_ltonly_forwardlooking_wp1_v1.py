...
vars_y_alt = ['allow_tot', 'prov_ratio']
vars_x = ['credex_tot', 'reg_cap', 'loanratio', 'roa', 'depratio',\
          'comloanratio', 'mortratio','consloanratio', 'loanhhi', 'costinc',\
          'size','bhc','intercept']
vars_instr = ['log_empl','log_num_branch','perc_limited_branch', 'log_states','log_empl_sqr','log_num_branch_sqr','perc_limited_branch_sqr', 'log_states_sqr']
...
if __name__ == '__main__':
    df_log = pd.concat(Parallel(n_jobs = num_cores)(delayed(logVars)(df, col) for col in vars_y + vars_y_alt + vars_x[:-1] + [vars_instr[2]]), axis = 1)
...
for var in vars_instr[:4]:
    df_log['{}_sqr'.format(var)] = df_log[var] ** 2
...
# Get a list of IDRSSDs from loan transferrers only
...
# Run
if __name__ == '__main__':
    ## Loan transferrers only
    results_benchmark_lt = Parallel(n_jobs = num_cores)(delayed(benchmarkModel)(df_fd[df_fd.index.get_level_values(0).isin(ls_idrssd)], elem, vars_rhs) for elem in vars_y)
    
    ## Alternative y vars
    results_benchmark_alt = Parallel(n_jobs = num_cores)(delayed(benchmarkModel)(df_fd, elem, vars_rhs) for elem in vars_y_alt)
...
z_lt = [vars_instr[0], vars_instr[1], vars_instr[4], vars_instr[5], \
        vars_trans[1], vars_trans[2], vars_trans[5], vars_trans[6]] # fix
z_alt = [vars_instr[0], vars_trans[1]]

# Run
if __name__ == '__main__':
    ## Loan transferrers only
    fdiv_stage_11_lt, fdiv_stage_12_lt, fdiv_stage_2_lt, f_11_lt, f_12_lt, dwh_lt, sh_lt = zip(*Parallel(n_jobs = num_cores)(delayed(FDIV)(df_fd[df_fd.index.get_level_values(0).isin(ls_idrssd)], df_z[df_z.index.get_level_values(0).isin(ls_idrssd)], elem, x_endo, x_exo, z_lt) for elem in vars_y))
    
    ## Other y vars
    fdiv_stage_11_alt, fdiv_stage_12_alt, fdiv_stage_2_alt, f_11_alt, f_12_alt, dwh_alt, sh_alt = zip(*Parallel(n_jobs = num_cores)(delayed(FDIV)(df_fd, df_z, elem, x_endo, x_exo, z_alt) for elem in vars_y_alt))
...
## Benchmark
results_benchmark_list_dfs_lt = []
results_benchmark_list_dfs_alt = []

for result in results_benchmark_lt:
    results_benchmark_list_dfs_lt.append(summaryToDFBenchmark(result))
    
for result in results_benchmark_alt:
    results_benchmark_list_dfs_alt.append(summaryToDFBenchmark(result))
    
## FDIV
### Step 1
fdiv_stage_1_list_dfs_lt = []
fdiv_stage_1_list_dfs_alt = []

fdiv_stage_1_list_dfs_lt.append(summaryToDFFDIV(fdiv_stage_11_lt[0], f = f_11_lt[0], stage = 0))
fdiv_stage_1_list_dfs_lt.append(summaryToDFFDIV(fdiv_stage_12_lt[0], f = f_12_lt[0], stage = 0))

fdiv_stage_1_list_dfs_alt.append(summaryToDFFDIV(fdiv_stage_11_alt[0], f = f_11_alt[0], stage = 0))
fdiv_stage_1_list_dfs_alt.append(summaryToDFFDIV(fdiv_stage_12_alt[0], f = f_12_alt[0], stage = 0))

### Step 2
fdiv_stage_2_list_dfs_lt = []
fdiv_stage_2_list_dfs_alt = []

for result, endo, sar in zip(fdiv_stage_2_lt, dwh_lt, sh_lt):
    fdiv_stage_2_list_dfs_lt.append(summaryToDFFDIV(result, endo = endo, sargan = sar, stage = 1))
    
for result, endo, sar in zip(fdiv_stage_2_alt, dwh_alt, sh_alt):
    fdiv_stage_2_list_dfs_alt.append(summaryToDFFDIV(result, endo = endo, sargan = sar, stage = 1))
    
# To latex
## FDIV Stage 1
### Prelims
col_label_step1_lt = ['({})'.format(i) for i in range(1,len(fdiv_stage_1_list_dfs_lt) + 1)]
caption_step1_lt = 'Robustness Check: Loan Transferrers Only (FDIV Model, First Stage)'
label_step1_lt = 'tab:robust_lt_step1'

col_label_step1_alt = ['({})'.format(i) for i in range(1,len(fdiv_stage_1_list_dfs_alt) + 1)]
caption_step1_alt = 'Robustness Check: Forward-looking Credit Risk (FDIV Model, First Stage)'
label_step1_alt = 'tab:robust_alt_step1'

## Get LaTeX results
df_results_step1_lt, latex_results_step1_lt = concatResults(fdiv_stage_1_list_dfs_lt, col_label = col_label_step1_lt,\
                                                  caption = caption_step1_lt, label = label_step1_lt, step = 0)
    
df_results_step1_alt, latex_results_step1_alt = concatResults(fdiv_stage_1_list_dfs_alt, col_label = col_label_step1_alt,\
                                                  caption = caption_step1_alt, label = label_step1_alt, step = 0)

# Benchmark and FDIV stage 2
col_label_step2_lt = ['Benchmark|({})'.format(i) for i in range(1,len(results_benchmark_list_dfs_lt) + 1)] +\
    ['FDIV|({})'.format(i + len(results_benchmark_list_dfs_lt)) for i in range(1,len(fdiv_stage_2_list_dfs_lt) + 1)]
caption_step2_lt = 'Robustness Check: Loan Transferrers Only (Second Stage)'
label_step2_lt = 'tab:robust_lt_step2'

# Benchmark and FDIV stage 2
col_label_step2_alt = ['Benchmark|({})'.format(i) for i in range(1,len(results_benchmark_list_dfs_alt) + 1)] +\
    ['FDIV|({})'.format(i + len(results_benchmark_list_dfs_alt)) for i in range(1,len(fdiv_stage_2_list_dfs_alt) + 1)]
caption_step2_alt = 'Robustness Check: Forward-looking Credit Risk (Second Stage)'
label_step2_alt = 'tab:robust_alt_step2'


## Get LaTeX results
df_results_step2_lt, latex_results_step2_lt = concatResults(results_benchmark_list_dfs_lt + fdiv_stage_2_list_dfs_lt, col_label = col_label_step2_lt,\
                                                  caption = caption_step2_lt, label = label_step2_lt, step = 1)
    
df_results_step2_alt, latex_results_step2_alt = concatResults(results_benchmark_list_dfs_alt + fdiv_stage_2_list_dfs_alt, col_label = col_label_step2_alt,\
                                                  caption = caption_step2_alt, label = label_step2_alt, step = 1)
...
# Step 1
df_results_step1_lt.to_csv('Robustness_checks/Step_1/Robust_lt_step1.csv')

text_file_latex_results_lt = open('Robustness_checks/Step_1/Robust_lt_step1.tex', 'w')
text_file_latex_results_lt.write(latex_results_step1_lt)
text_file_latex_results_lt.close()

df_results_step1_alt.to_csv('Robustness_checks/Step_1/Robust_alt_step1.csv')

text_file_latex_results_alt = open('Robustness_checks/Step_1/Robust_alt_step1.tex', 'w')
text_file_latex_results_alt.write(latex_results_step1_alt)
text_file_latex_results_alt.close()

# Step 2
df_results_step2_lt.to_csv('Robustness_checks/Step_2/Robust_lt_step2.csv')

text_file_latex_results_lt = open('Robustness_checks/Step_2/Robust_lt_step2.tex', 'w')
text_file_latex_results_lt.write(latex_results_step2_lt)
text_file_latex_results_lt.close()

df_results_step2_alt.to_csv('Robustness_checks/Step_2/Robust_alt_step2.csv')

text_file_latex_results_alt = open('Robustness_checks/Step_2/Robust_alt_step2.tex', 'w')
text_file_latex_results_alt.write(latex_results_step2_alt)
text_file_latex_results_alt.close()