...
df_sub_fd = df_sub.groupby(df_sub.index.get_level_values(0)).transform(lambda df: df.shift(periods = 1) - df).dropna()

## Add dummies
dummy_full_fd = pd.get_dummies(df_full_fd.index.get_level_values(1))
dummy_sub_fd = pd.get_dummies(df_sub_fd.index.get_level_values(1))
...
dummy_sub_fd = pd.DataFrame(np.array(dummy_sub_fd), index = df_sub_fd.index, columns = col_dummy)
df_full_fd[col_dummy] = dummy_full_fd
df_sub_fd[col_dummy] = dummy_sub_fd
...
if log:
    rank_full = np.linalg.matrix_rank(df_full_fd[[\
...
## Dummy LS
res_full_step1_w = PanelOLS.from_formula('dum_ls' + ' ~ ' + righthand_x + ' + ' + righthand_z + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_step1_w.summary)
df_full_fd['G_hat_fd_w'] = res_full_step1_w.fitted_values

## LS/TA
res_full_step1_ls = PanelOLS.from_formula('ls_tot_ta' + ' ~ ' + righthand_x + ' + ' + righthand_z + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_step1_ls.summary)
df_full_fd['G_hat_fd_ls'] = res_full_step1_ls.fitted_values

#----------------------------------------------
# Calculate G_hat_x_xbar for both first stages
G_hat_x_xbar_fd_w = df_full_fd.loc[:,df_full_fd.columns.str.contains('_xbar')] * df_full_fd.G_hat_fd_w[:, None]
if log:
    df_full_fd[[x + '_G_hat_w' for x in ['RC7205','loanratio','roa',\
                                         'depratio','comloanratio','RC2170','bhc']]] = G_hat_x_xbar_fd_w
else:
    df_full_fd[[x + '_G_hat_w' for x in ['RC7205','loanratio','roa',\
                                         'depratio','comloanratio','size','bhc']]] = G_hat_x_xbar_fd_w

G_hat_x_xbar_fd_ls = df_full_fd.loc[:,df_full_fd.columns.str.contains('_xbar')] * df_full_fd.G_hat_fd_ls[:, None]
if log:
    df_full_fd[[x + '_G_hat_ls' for x in ['RC7205','loanratio','roa',\
...
#----------------------------------------------
## Charge off 
res_full_step2a_charge_w = PanelOLS.from_formula('net_coffratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_w' + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_step2a_charge_w.summary)

res_full_step2b_charge_w = PanelOLS.from_formula('net_coffratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_w' + '+' + righthand_ghat_w + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_step2b_charge_w.summary)

#----------------------------------------------
## Allowance 
res_full_step2a_allow_w = PanelOLS.from_formula('allowratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_w' + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_step2a_allow_w.summary)

res_full_step2b_allow_w = PanelOLS.from_formula('allowratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_w' + '+' + righthand_ghat_w + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_step2b_allow_w.summary)

#----------------------------------------------
## rwata 
res_full_step2a_rwata_w = PanelOLS.from_formula('rwata' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_w' + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_step2a_rwata_w.summary)

res_full_step2b_rwata_w = PanelOLS.from_formula('rwata' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_w' + '+' + righthand_ghat_w + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_step2b_rwata_w.summary)

#----------------------------------------------
## prov 
res_full_step2a_prov_w = PanelOLS.from_formula('provratio' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_w' + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_step2a_prov_w.summary)

res_full_step2b_prov_w = PanelOLS.from_formula('provratio' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_w' + '+' + righthand_ghat_w + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
...
#----------------------------------------------
## Charge off 
res_full_step2a_charge_ls = PanelOLS.from_formula('net_coffratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_ls' + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_step2a_charge_ls.summary)

res_full_step2b_charge_ls = PanelOLS.from_formula('net_coffratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_ls' + '+' + righthand_ghat_ls + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_step2b_charge_ls.summary)

#----------------------------------------------
## Allowance
res_full_step2a_allow_ls = PanelOLS.from_formula('allowratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_ls' + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_step2a_allow_ls.summary)

res_full_step2b_allow_ls = PanelOLS.from_formula('allowratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_ls' + '+' + righthand_ghat_ls + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_step2b_allow_ls.summary)

#----------------------------------------------
## rwata
res_full_step2a_rwata_ls = PanelOLS.from_formula('rwata' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_ls' + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_step2a_rwata_ls.summary)

res_full_step2b_rwata_ls = PanelOLS.from_formula('rwata' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_ls' + '+' + righthand_ghat_ls + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_step2b_rwata_ls.summary)

#----------------------------------------------
## prov
res_full_step2a_prov_ls = PanelOLS.from_formula('provratio' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_ls' + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_step2a_prov_ls.summary)

res_full_step2b_prov_ls = PanelOLS.from_formula('provratio' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_ls' + '+' + righthand_ghat_ls + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
...
## Weak instruments
### Dummy LS
res_full_step1b_w = PanelOLS.from_formula('dum_ls' + ' ~ ' + righthand_x + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
f_test_full_step1b_w = fTestWeakInstruments(df_full_fd.dum_ls, res_full_step1_w.fitted_values, res_full_step1b_w.fitted_values, num_z) #11.141455804993024

### LS/TA
res_full_step1b_ls = PanelOLS.from_formula('ls_tot_ta' + ' ~ ' + righthand_x + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
f_test_full_step1b_ls = fTestWeakInstruments(df_full_fd.ls_tot_ta, res_full_step1_ls.fitted_values, res_full_step1b_ls.fitted_values, num_z) #17.200081157277296
...
#----------------------------------------------
### Dummy LS
df_full_fd['resid_step1_w'] = res_full_step1_w.resids

#----------------------------------------------
#### Charge-off
res_full_charge_w_endo = PanelOLS.from_formula('net_coffratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'dum_ls' + ' + ' + 'resid_step1_w' + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_charge_w_endo.summary) # p-val = 0.0407

#----------------------------------------------
#### Allow
res_full_allow_w_endo = PanelOLS.from_formula('allowratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'dum_ls' + ' + ' + 'resid_step1_w' + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_allow_w_endo.summary) # p-val = 0.0000

#----------------------------------------------
#### rwata
res_full_rwata_w_endo = PanelOLS.from_formula('rwata' + ' ~ ' + righthand_x + ' + ' + 'dum_ls' + ' + ' + 'resid_step1_w' + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_rwata_w_endo.summary) # p-val = 0.0000

#----------------------------------------------
#### prov
res_full_prov_w_endo = PanelOLS.from_formula('provratio' + ' ~ ' + righthand_x + ' + ' + 'dum_ls' + ' + ' + 'resid_step1_w' + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_prov_w_endo.summary) # p-val = 0.0681

#----------------------------------------------
### LS/TA
df_full_fd['resid_step1_ls'] = res_full_step1_ls.resids

#----------------------------------------------
#### Charge-off
res_full_charge_ls_endo = PanelOLS.from_formula('net_coffratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'ls_tot_ta' + ' + ' + 'resid_step1_ls' + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_charge_ls_endo.summary) # p-val = 0.0296

#----------------------------------------------
#### Allow
res_full_allow_ls_endo = PanelOLS.from_formula('allowratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'ls_tot_ta' + ' + ' + 'resid_step1_ls' + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_allow_ls_endo.summary) # p-val = 0.0000

#----------------------------------------------
#### rwata
res_full_rwata_ls_endo = PanelOLS.from_formula('rwata' + ' ~ ' + righthand_x + ' + ' + 'ls_tot_ta' + ' + ' + 'resid_step1_ls' + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_full_rwata_ls_endo.summary) # p-val = 0.0000

#----------------------------------------------
#### prov
res_full_prov_ls_endo = PanelOLS.from_formula('provratio' + ' ~ ' + righthand_x + ' + ' + 'ls_tot_ta' + ' + ' + 'resid_step1_ls' + ' + ' + time_dummies, data = df_full_fd).fit(cov_type = 'clustered', cluster_entity = True)
...
#----------------------------------------------
#### Charge-off
oir_full_step2a_charge_w = sargan(res_full_step2a_charge_w.resids, df_full_fd[righthand_x.split(' + ')], df_full_fd[righthand_z.split(' + ')]) # p-val = 0.0000
oir_full_step2b_charge_w = sargan(res_full_step2b_charge_w.resids, df_full_fd[righthand_x.split(' + ')], df_full_fd[righthand_z.split(' + ')]) # p-val = 0.0000

#----------------------------------------------
#### Allow
oir_full_step2a_allow_w = sargan(res_full_step2a_allow_w.resids, df_full_fd[righthand_x.split(' + ')], df_full_fd[righthand_z.split(' + ')]) # p-val = 0.0000
oir_full_step2b_allow_w = sargan(res_full_step2b_allow_w.resids, df_full_fd[righthand_x.split(' + ')], df_full_fd[righthand_z.split(' + ')]) # p-val = 0.0000

#----------------------------------------------
#### rwata
oir_full_step2a_rwata_w = sargan(res_full_step2a_rwata_w.resids, df_full_fd[righthand_x.split(' + ')], df_full_fd[righthand_z.split(' + ')]) # p-val = 0.0023
oir_full_step2b_rwata_w = sargan(res_full_step2b_rwata_w.resids, df_full_fd[righthand_x.split(' + ')], df_full_fd[righthand_z.split(' + ')]) # p-val = 0.0015

#----------------------------------------------
#### prov
oir_full_step2a_prov_w = sargan(res_full_step2a_prov_w.resids, df_full_fd[righthand_x.split(' + ')], df_full_fd[righthand_z.split(' + ')]) # p-val = 0.6458
oir_full_step2b_prov_w = sargan(res_full_step2b_prov_w.resids, df_full_fd[righthand_x.split(' + ')], df_full_fd[righthand_z.split(' + ')]) # p-val = 0.5889
...
#----------------------------------------------
#### Charge-off
oir_full_step2a_charge_ls = sargan(res_full_step2a_charge_ls.resids, df_full_fd[righthand_x.split(' + ')], df_full_fd[righthand_z.split(' + ')]) # p-val = 0.0049
oir_full_step2b_charge_ls = sargan(res_full_step2b_charge_ls.resids, df_full_fd[righthand_x.split(' + ')], df_full_fd[righthand_z.split(' + ')]) # p-val = 0.0063

#----------------------------------------------
#### Allow
oir_full_step2a_allow_ls = sargan(res_full_step2a_allow_ls.resids, df_full_fd[righthand_x.split(' + ')], df_full_fd[righthand_z.split(' + ')]) # p-val = 0.0058
oir_full_step2b_allow_ls = sargan(res_full_step2b_allow_ls.resids, df_full_fd[righthand_x.split(' + ')], df_full_fd[righthand_z.split(' + ')]) # p-val = 0.0069

#----------------------------------------------
#### rwata
oir_full_step2a_rwata_ls = sargan(res_full_step2a_rwata_ls.resids, df_full_fd[righthand_x.split(' + ')], df_full_fd[righthand_z.split(' + ')]) # p-val = 0.0027
oir_full_step2b_rwata_ls = sargan(res_full_step2b_rwata_ls.resids, df_full_fd[righthand_x.split(' + ')], df_full_fd[righthand_z.split(' + ')]) # p-val = 0.0030

#----------------------------------------------
#### prov
oir_full_step2a_prov_ls = sargan(res_full_step2a_prov_ls.resids, df_full_fd[righthand_x.split(' + ')], df_full_fd[righthand_z.split(' + ')]) # p-val = 0.8760
oir_full_step2b_prov_ls = sargan(res_full_step2b_prov_ls.resids, df_full_fd[righthand_x.split(' + ')], df_full_fd[righthand_z.split(' + ')]) # p-val = 0.9156
...
# First check the data on column rank
if log:
    rank_sub = np.linalg.matrix_rank(df_sub_fd[[\
                              'RC7205','loanratio','roa','depratio','comloanratio',\
                              'RC2170','num_branch','bhc']]) # 10 (should be 10)
else:
    rank_sub = np.linalg.matrix_rank(df_sub_fd[[\
...
# Estimate G_hat             
## Dummy LS
res_sub_step1_w = PanelOLS.from_formula('dum_ls' + ' ~ ' + righthand_x + ' + ' + righthand_z + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_step1_w.summary)
df_sub_fd['G_hat_fd_w'] = res_sub_step1_w.fitted_values

## LS/TA
res_sub_step1_ls = PanelOLS.from_formula('ls_tot_ta' + ' ~ ' + righthand_x + ' + ' + righthand_z + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_step1_ls.summary)
df_sub_fd['G_hat_fd_ls'] = res_sub_step1_ls.fitted_values

#----------------------------------------------
# Calculate G_hat_x_xbar for both first stages
G_hat_x_xbar_fd_w = df_sub_fd.loc[:,df_sub_fd.columns.str.contains('_xbar')] * df_sub_fd.G_hat_fd_w[:, None]
if log:
    df_sub_fd[[x + '_G_hat_w' for x in ['RC7205','loanratio','roa',\
                                    'depratio','comloanratio','RC2170','bhc']]] = G_hat_x_xbar_fd_w
else:
    df_sub_fd[[x + '_G_hat_w' for x in ['RC7205','loanratio','roa',\
                                    'depratio','comloanratio','size','bhc']]] = G_hat_x_xbar_fd_w

G_hat_x_xbar_fd_ls = df_sub_fd.loc[:,df_sub_fd.columns.str.contains('_xbar')] * df_sub_fd.G_hat_fd_ls[:, None]
if log:
    df_sub_fd[[x + '_G_hat_ls' for x in ['RC7205','loanratio','roa',\
                                     'depratio','comloanratio','RC2170','bhc']]] = G_hat_x_xbar_fd_ls
else:
    df_sub_fd[[x + '_G_hat_ls' for x in ['RC7205','loanratio','roa',\
                                     'depratio','comloanratio','size','bhc']]] = G_hat_x_xbar_fd_ls
...
#----------------------------------------------
## Charge off 
res_sub_step2a_charge_w = PanelOLS.from_formula('net_coffratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_w' + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_step2a_charge_w.summary)

res_sub_step2b_charge_w = PanelOLS.from_formula('net_coffratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_w' + '+' + righthand_ghat_w + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_step2b_charge_w.summary)

#----------------------------------------------
## Allowance 
res_sub_step2a_allow_w = PanelOLS.from_formula('allowratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_w' + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_step2a_allow_w.summary)

res_sub_step2b_allow_w = PanelOLS.from_formula('allowratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_w' + '+' + righthand_ghat_w + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_step2b_allow_w.summary)

#----------------------------------------------
## rwata 
res_sub_step2a_rwata_w = PanelOLS.from_formula('rwata' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_w' + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_step2a_rwata_w.summary)

res_sub_step2b_rwata_w = PanelOLS.from_formula('rwata' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_w' + '+' + righthand_ghat_w + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_step2b_rwata_w.summary)

#----------------------------------------------
## prov 
res_sub_step2a_prov_w = PanelOLS.from_formula('provratio' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_w' + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_step2a_prov_w.summary)

res_sub_step2b_prov_w = PanelOLS.from_formula('provratio' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_w' + '+' + righthand_ghat_w + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
...
#----------------------------------------------
## Charge off 
res_sub_step2a_charge_ls = PanelOLS.from_formula('net_coffratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_ls' + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_step2a_charge_ls.summary)

res_sub_step2b_charge_ls = PanelOLS.from_formula('net_coffratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_ls' + '+' + righthand_ghat_ls + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_step2b_charge_ls.summary)

#----------------------------------------------
## Allowance
res_sub_step2a_allow_ls = PanelOLS.from_formula('allowratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_ls' + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_step2a_allow_ls.summary)

res_sub_step2b_allow_ls = PanelOLS.from_formula('allowratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_ls' + '+' + righthand_ghat_ls + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_step2b_allow_ls.summary)

#----------------------------------------------
## rwata
res_sub_step2a_rwata_ls = PanelOLS.from_formula('rwata' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_ls' + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_step2a_rwata_ls.summary)

res_sub_step2b_rwata_ls = PanelOLS.from_formula('rwata' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_ls' + '+' + righthand_ghat_ls + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_step2b_rwata_ls.summary)

#----------------------------------------------
## prov
res_sub_step2a_prov_ls = PanelOLS.from_formula('provratio' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_ls' + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_step2a_prov_ls.summary)

res_sub_step2b_prov_ls = PanelOLS.from_formula('provratio' + ' ~ ' + righthand_x + ' + ' + 'G_hat_fd_ls' + '+' + righthand_ghat_ls + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
...
## Weak instruments
### Dummy LS
res_sub_step1b_w = PanelOLS.from_formula('dum_ls' + ' ~ ' + righthand_x + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
f_test_sub_step1b_w = fTestWeakInstruments(df_sub_fd.dum_ls, res_sub_step1_w.fitted_values, res_sub_step1b_w.fitted_values, num_z) #0.7411053849984024

### LS/TA
res_sub_step1b_ls = PanelOLS.from_formula('ls_tot_ta' + ' ~ ' + righthand_x + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
f_test_sub_step1b_ls = fTestWeakInstruments(df_sub_fd.ls_tot_ta, res_sub_step1_ls.fitted_values, res_sub_step1b_ls.fitted_values, num_z) #12.158629955639562
...
#----------------------------------------------
### Dummy LS
df_sub_fd['resid_step1_w'] = res_sub_step1_w.resids

#----------------------------------------------
#### Charge-off
res_sub_charge_w_endo = PanelOLS.from_formula('net_coffratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'dum_ls' + ' + ' + 'resid_step1_w' + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_charge_w_endo.summary) # p-val = 0.5143

#----------------------------------------------
#### Allow
res_sub_allow_w_endo = PanelOLS.from_formula('allowratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'dum_ls' + ' + ' + 'resid_step1_w' + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_allow_w_endo.summary) # p-val = 0.0654

#----------------------------------------------
#### rwata
res_sub_rwata_w_endo = PanelOLS.from_formula('rwata' + ' ~ ' + righthand_x + ' + ' + 'dum_ls' + ' + ' + 'resid_step1_w' + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_rwata_w_endo.summary) # p-val = 0.0183

#----------------------------------------------
#### prov
res_sub_prov_w_endo = PanelOLS.from_formula('provratio' + ' ~ ' + righthand_x + ' + ' + 'dum_ls' + ' + ' + 'resid_step1_w' + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_prov_w_endo.summary) # p-val = 0.9713


#----------------------------------------------
### LS/TA
df_sub_fd['resid_step1_ls'] = res_sub_step1_ls.resids

#----------------------------------------------
#### Charge-off
res_sub_charge_ls_endo = PanelOLS.from_formula('net_coffratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'ls_tot_ta' + ' + ' + 'resid_step1_ls' + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_charge_ls_endo.summary) # p-val = 0.1515

#----------------------------------------------
#### Allow
res_sub_allow_ls_endo = PanelOLS.from_formula('allowratio_tot_ta' + ' ~ ' + righthand_x + ' + ' + 'ls_tot_ta' + ' + ' + 'resid_step1_ls' + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_allow_ls_endo.summary) # p-val = 0.7587

#----------------------------------------------
#### rwata
res_sub_rwata_ls_endo = PanelOLS.from_formula('rwata' + ' ~ ' + righthand_x + ' + ' + 'ls_tot_ta' + ' + ' + 'resid_step1_ls' + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
print(res_sub_rwata_ls_endo.summary) # p-val = 0.1396

#----------------------------------------------
#### prov
res_sub_prov_ls_endo = PanelOLS.from_formula('provratio' + ' ~ ' + righthand_x + ' + ' + 'ls_tot_ta' + ' + ' + 'resid_step1_ls' + ' + ' + time_dummies, data = df_sub_fd).fit(cov_type = 'clustered', cluster_entity = True)
...
#----------------------------------------------
#### Charge-off
oir_sub_step2a_charge_w = sargan(res_sub_step2a_charge_w.resids, df_sub_fd[righthand_x.split(' + ')], df_sub_fd[righthand_z.split(' + ')]) # p-val = 0.2873
oir_sub_step2b_charge_w = sargan(res_sub_step2b_charge_w.resids, df_sub_fd[righthand_x.split(' + ')], df_sub_fd[righthand_z.split(' + ')]) # p-val = 0.2289

#----------------------------------------------
#### Allow
oir_sub_step2a_allow_w = sargan(res_sub_step2a_allow_w.resids, df_sub_fd[righthand_x.split(' + ')], df_sub_fd[righthand_z.split(' + ')]) # p-val = 0.2772
oir_sub_step2b_allow_w = sargan(res_sub_step2b_allow_w.resids, df_sub_fd[righthand_x.split(' + ')], df_sub_fd[righthand_z.split(' + ')]) # p-val = 0.1291

#----------------------------------------------
#### rwata
oir_sub_step2a_rwata_w = sargan(res_sub_step2a_rwata_w.resids, df_sub_fd[righthand_x.split(' + ')], df_sub_fd[righthand_z.split(' + ')]) # p-val = 0.4020
oir_sub_step2b_rwata_w = sargan(res_sub_step2b_rwata_w.resids, df_sub_fd[righthand_x.split(' + ')], df_sub_fd[righthand_z.split(' + ')]) # p-val = 0.2363

#----------------------------------------------
#### prov
oir_sub_step2a_prov_w = sargan(res_sub_step2a_prov_w.resids, df_sub_fd[righthand_x.split(' + ')], df_sub_fd[righthand_z.split(' + ')]) # p-val = 0.1869
oir_sub_step2b_prov_w = sargan(res_sub_step2b_prov_w.resids, df_sub_fd[righthand_x.split(' + ')], df_sub_fd[righthand_z.split(' + ')]) # p-val = 0.1852
...
#----------------------------------------------
#### Charge-off
oir_sub_step2a_charge_ls = sargan(res_sub_step2a_charge_ls.resids, df_sub_fd[righthand_x.split(' + ')], df_sub_fd[righthand_z.split(' + ')]) # p-val = 0.7704
oir_sub_step2b_charge_ls = sargan(res_sub_step2b_charge_ls.resids, df_sub_fd[righthand_x.split(' + ')], df_sub_fd[righthand_z.split(' + ')]) # p-val = 0.0313

#----------------------------------------------
#### Allow
oir_sub_step2a_allow_ls = sargan(res_sub_step2a_allow_ls.resids, df_sub_fd[righthand_x.split(' + ')], df_sub_fd[righthand_z.split(' + ')]) # p-val = 0.0108
oir_sub_step2b_allow_ls = sargan(res_sub_step2b_allow_ls.resids, df_sub_fd[righthand_x.split(' + ')], df_sub_fd[righthand_z.split(' + ')]) # p-val = 0.0313

#----------------------------------------------
#### rwata
oir_sub_step2a_rwata_ls = sargan(res_sub_step2a_rwata_ls.resids, df_sub_fd[righthand_x.split(' + ')], df_sub_fd[righthand_z.split(' + ')]) # p-val = 0.0004
oir_sub_step2b_rwata_ls = sargan(res_sub_step2b_rwata_ls.resids, df_sub_fd[righthand_x.split(' + ')], df_sub_fd[righthand_z.split(' + ')]) # p-val = 0.0001

#----------------------------------------------
#### prov
oir_sub_step2a_prov_ls = sargan(res_sub_step2a_prov_ls.resids, df_sub_fd[righthand_x.split(' + ')], df_sub_fd[righthand_z.split(' + ')]) # p-val = 0.4849
oir_sub_step2b_prov_ls = sargan(res_sub_step2b_prov_ls.resids, df_sub_fd[righthand_x.split(' + ')], df_sub_fd[righthand_z.split(' + ')]) # p-val = 0.3829
...
if log:
    if change_ls:
        with pd.ExcelWriter('FD_IV_results.xlsx') as writer:
