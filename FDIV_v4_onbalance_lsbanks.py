...
vars_needed = ['distance','provratio','net_coffratio_on_ta',\
               'allowratio_on_ta','ls_tot_ta','dum_ls','size',\
...
ls_banks_ids = df_full[df_full.ls_tot_ta > 0.0].index.get_level_values(0).unique().tolist()
df = df_full[df_full.index.get_level_values(0).isin(ls_banks_ids)]

## Take logs of the df
df = df.transform(lambda df: np.log(1 + df))

## Take the first differences
df_fd = df.groupby(df.index.get_level_values(0)).diff(periods = 1).dropna()
...
def analysesFDIV(df, var_ls, righthand_x, righthand_z, time_dummies):
    ...
    dep_var_step2 = ['net_coffratio_on_ta','allowratio_on_ta','provratio']
...
# Save the tables
path = r'Results\FD_IV_v4_log_onbalance_lsbanks.xlsx'
path_pcorr = r'Results\partial_corr_log_onbalance_lsbanks.xlsx'
path_pr2 = r'Results\partial_pr2_log_onbalance_lsbanks.xlsx'
