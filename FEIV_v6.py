...
def fixedEffects(group):
    return(group - group.mean())

df_grouped = df[righthand_x.split(' + ') + [vars_endo] + vars_z.split(' + ') + dep_vars].groupby(df.index.get_level_values(0))
    
if __name__ == '__main__':
    df_fd = pd.concat(Parallel(n_jobs = num_cores)(delayed(fixedEffects)(group) for name, group in df_grouped))
...
''' Crisis dates are (based on the NBER recession dates):
    ...
    '''
df_pre = df_fd[df_fd.index.get_level_values(1) <= pd.Timestamp(2006,12,31)]
df_during = df_fd[(df_fd.index.get_level_values(1) > pd.Timestamp(2006,12,31)) & (df_fd.index.get_level_values(1) < pd.Timestamp(2010,12,31))]
df_post = df_fd[df_fd.index.get_level_values(1) >= pd.Timestamp(2010,12,31)]
df_dodd = df_fd[df_fd.index.get_level_values(1) < pd.Timestamp(2010,12,31)]
...
def analysesFDIV(dep_var, var_ls, df, righthand_z, righthand_x):
    ...
    # Estimate G_hat
    res_step1 = PanelOLS.from_formula(var_ls + ' ~ ' + righthand_x + ' + ' + righthand_z + ' + ' + time_dummies_step1,\
...
list_dfs = [df_fd, df_pre, df_during, df_post, df_dodd]

# Make an iterable list
iter_list = itertools.product(list_dfs, dep_vars)

#----------------------------------------------
# Run models
#----------------------------------------------
if __name__ == '__main__': 
    step1, step2, f, endo, sargan_res = zip(*Parallel(n_jobs = num_cores)(delayed(analysesFDIV)(dep_var, vars_endo, data, vars_z, righthand_x) for data in list_dfs for dep_var in dep_vars))
...
def step1ToCSV(table1, fval, i):    
    ...
    ## Save to csv
    main_table1.to_csv('Results\FE\Step_1\Step1_fe_{}.csv'.format(i))
...
def step2ToCSV(table2, endo_val, sargan_val, i):    
    ...
    ## Save to csv
    main_table2.to_csv('Results\FE\Step_2\Step2_fe_{}.csv'.format(i))
